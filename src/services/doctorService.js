...
let getDetailDoctorByIds = (inputId) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!inputId) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!data.arrSchedule || !data.doctorId || !data.formateDate) {
        ...
        // kiểm tra xem đã có trong data chưa rồi bỏ ra ( rất khó )
  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (!doctorId) {
        resolve({
          errCode: 1,
          errMessage: "Missing required doctorId",
        });
      } else {
...
let getListPatientForDoctors = (doctorId, date) => {
  return new Promise(async (resolve, reject) => {
    try {
      if (!doctorId || !date) {
        resolve({
...
module.exports = {
  ...
  getDetailDoctorByIds,
  bulkCreateSchedule,
  getScheduleByDate,
  getExtraInforDoctorById,
  getProfileDoctorById,
  getListPatientForDoctors,
};
